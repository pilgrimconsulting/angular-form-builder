...
.directive 'fbFormObjectEditable', ['$injector', ($injector) ->
	...
	link: (scope, element) ->
		...
		# setup scope
		scope.setupScope scope.formObject, scope.componentName
...
#		return if not scope.formObject.editable
		...
		scope.popover =
			...
			remove: ($event) ->
				...
				console.log('TRY TO DELETE', scope.componentName)
#				if scope.componentName == 'section'
		...
		<div class='form-horizontal col-sm-12 elementList'>
			<div ng-repeat="component in components">
				<div class="form-group element-wrapper">
					...
					<div class="col-sm-11">
						<div class='fb-component' fb-component="component" ng-component="{{component.name}}" ng-if='!$builder.simplePreview'></div>
...
#		sectionIndex = $(element).closest(".fb-form-object-editable").index()
		...
		console.log('Init Section: ',sectionIndex,scope.sectionObjects )
		#		console.log "--------------> builder"
		#		console.log $builder.forms
		# $drag.draggable $(element),
		...
		$drag.droppable $(element),
			...
			up: (e, isHover, draggable) ->
				...
				if not isHover and draggable.mode is 'drag'
					...
					# if formObject.editable
					console.log('removeSectionObject', attrs.fbBuilder, sectionIndex, formObject.index, formObject.editable, formObject, draggable.object.formObject)
#						$(element).find('.fb-form-object-editable')
#					$builder.removeSectionObject attrs.fbBuilder, sectionIndex, formObject.index
				else if isHover
					# Add or Sort components
					if draggable.mode is 'mirror'
						# insert a form object
						elementIndex = $(element).find('.empty').index()
						console.warn('insertSectionObject',sectionIndex,elementIndex)
						$builder.insertSectionObject $builder.currentForm, sectionIndex, elementIndex,
							component: draggable.object.componentName
						scope.sectionObjects = $builder.getSectionObjects sectionIndex
...
#						sectionIndex = $(element).closest(".fb-form-object-editable").index()
						# SORTING - update
#						console.log('updateSectionObjectIndex',scope.formNumber, sectionIndex, oldIndex, newIndex)
						$builder.updateSectionObjectIndex scope.formNumber, sectionIndex, oldIndex, newIndex
				$(element).find('.empty').remove()
]
...
#		0: []
	...
	@getSectionObjects = (sectionIndex) =>
		console.log('@getSectionObjects',sectionIndex,@forms[@currentForm][sectionIndex].components)
		if @forms[@currentForm][sectionIndex]
			@forms[@currentForm][sectionIndex].components
		else []
